  function X=stft_multi(x,wlen,overlapping)
...
if wlen~=4*floor(wlen/4), error('The window length must be a multiple of 4.'); end %floor（向下取整）
...
% Zero-padding(补零)
nfram=ceil(nsampl/overlapping); %ceil（向上取整）帧数
x=[x,zeros(nchan,nfram*wlen/2-nsampl)];
% Pre-processing for edges(初始化)
x=[zeros(nchan,wlen/4),x,zeros(nchan,wlen/4)];
swin=zeros((nfram+1)*wlen/2,1);
for t=0:nfram-1, %一帧一帧进行处理，循环
    swin(t*overlapping+1:t*overlapping+wlen)=swin(t*overlapping+1:t*overlapping+wlen)+win.^2;
end
swin=sqrt(wlen*swin);
nbin=wlen/2+1;
X=zeros(nbin,nfram,nchan); %(nbin,帧数，信道)
for i=1:nchan,
    for t=0:nfram-1,
        % Framing
        frame=x(i,t*overlapping+1:t*overlapping+wlen).'.*win./swin(t*overlapping+1:t*overlapping+wlen); 
        % FFT
        fframe=fft(frame);
